...
def textAnalysis(paramList):
    ...
    print('%%%%\nUSING ' + sys.argv[2] + ' KEYWORDS')
    loTest.writeKeywords(sys.argv[2],targetWordCount,startCount)
...
if __name__ == '__main__':
    ...
    # set folder where raw text files are
    ...
    #testSplit=.3
    targetWordCount=int(sys.argv[3])
    svdInt=50
    simCount=1000
    ...
    runMaster(rawPath,runDirectory, paramPath,runID,targetWordCount,startCount,cocoWindow,svdInt,cvWindow,netAngle,simCount)
...
#####################
### THIS SHOULDN'T RUN, BUT I PUT THIS CONDITIONAL IN INSTEAD OF DELETING IT ALL, in case we want it later
#######
if sys.argv[1] == 'runModel':
